<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="movieInfo.model.vo.*"%>
    
<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%
	MovieInfo m = (MovieInfo)request.getAttribute("m");
	MovieFile f = (MovieFile)request.getAttribute("f");
...
<body>
		...
		<main>
			...
			<form action="<%= request.getContextPath() %>/movieUpdate.mv" method="post" encType="multipart/form-data">
					<input type ="hidden" name = "no" value="<%=m.getMovieNo() %>">
					<input type ="hidden" name = "thumbnail" value="<%= f.getChangeName()%>">
			...
			<div class="movieInfo">
				<div class="movieInfo_view">
					...
					<tr>
						...
						<td colspan="3">
							<div id="titleImgArea">
								<img id="titleImg" width="350" height="200" src="<%= request.getContextPath() %>/uploadFiles/<%= f.getChangeName()%>">
							</div>
						</td>
					</tr>
						<tr>
							<th width="100px">제목</th>
							<td colspan="3"><input type="text" size="15" name="title" value="<%= m.getMovieTitle() %>"></td>
						</tr>
						<tr>
							<th width="100px">개봉일</th>
							<td colspan="3"><input type="text" size="15" name="date" value="<%= m.getMovieDate() %>"></td>
						</tr>
						<tr>
							<th width="100px">감독</th>
							<td colspan="10"><input type="text" size="15" name="director" value="<%=m.getDirector() %>"></td>
						</tr>
						<tr>
							<th width="100px">출연진</th>
							<td colspan="10"><input type="text" size="15" name="actor" value="<%=m.getActor() %>"></td>
						</tr>
						<tr>
							<th width="100px">장르</th>
							<td colspan="3"><input type="text" size="15" name="genre" value="<%=m.getGenre() %>"></td>
						</tr>
						<tr>
							<th width="100px">상영시간</th>
							<td colspan="3"><input type="text" size="15" name="runningTime" value="<%=m.getRunningTime() %>"></td>
						</tr>
						<tr>
							<th width="100px">관람등급</th>
							<td colspan="3"><input type="text" size="15" name="age" value="<%=m.getAge() %>"></td>
						</tr>
						<tr>
							<th width="100px">줄거리</th>
							<td colspan="3"><textarea name="content" rows="30" cols="60" style="resize:none;"><%=m.getContent() %></textarea>
						</tr>
					<tr>
						<th>스틸컷</th>
						<td>
