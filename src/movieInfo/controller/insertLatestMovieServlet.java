...
/**
 * Servlet implementation class insertLatestMovieServlet
 */
@WebServlet("/lMovieInsert")
public class insertLatestMovieServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public insertLatestMovieServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) { 
			...
			int result = new MovieInfoService().insertLMovie(m, fileList);
			
			
			if(result > 0) {
				response.sendRedirect("listLMovie");
			}else {
