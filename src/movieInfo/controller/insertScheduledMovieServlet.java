...
/**
 * Servlet implementation class insertScheduledMovieServlet
 */
@WebServlet("/SMovieInsert")
public class insertScheduledMovieServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public insertScheduledMovieServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(ServletFileUpload.isMultipartContent(request)) { 
			...
			int result = new MovieInfoService().insertSMovie(m, fileList);
			
			
			if(result > 0) {
				response.sendRedirect("listSMovie");
			}else {
